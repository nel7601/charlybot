	...
	async function checkInitialState() {
		try {
			...
			if (data.activeCocktailId) {
				...
				console.log(
					`[Initial State] Resuming monitoring for ${data.activeCocktailId}`,
				);
			...
			errorMessage =
				"Failed to connect to robot. Please check connection.";
		} finally {
			initializing = false;
		}
	}
	...
	async function handleCustomCocktailOrder(ingredients) {
		...
		try {
			...
			if (!response.ok) {
				const error = await response.json();
				throw new Error(
					error.message || "Failed to order custom cocktail",
				);
			}
...
<div class="min-h-screen bg-gradient-to-br from-cyan-50 via-teal-50 to-blue-50">
	...
	<div class="fixed inset-0 overflow-hidden pointer-events-none">
		<div
			class="absolute top-0 left-1/4 w-96 h-96 bg-cyan-400/10 rounded-full blur-3xl animate-pulse"
		></div>
		<div
			class="absolute bottom-0 right-1/4 w-96 h-96 bg-teal-400/10 rounded-full blur-3xl"
			style="animation: float 6s ease-in-out infinite;"
		></div>
		<div
			class="absolute top-1/2 right-1/3 w-72 h-72 bg-blue-400/5 rounded-full blur-3xl"
	...
	<!-- Header -->
	<header
		class="relative backdrop-blur-md bg-white/80 border-b border-cyan-200/50 shadow-sm"
	>
		<div class="container mx-auto px-8 py-6">
			<div class="flex items-center justify-between">
				<div class="flex items-center gap-6">
					<img
						src="/image/krka-logo.webp"
						alt="KRKA Power Inc"
						class="h-16 md:h-20 w-auto"
					/>
					<div
						class="h-12 md:h-14 w-px bg-gradient-to-b from-transparent via-cyan-300 to-transparent"
					></div>
	...
	{#if errorMessage}
		<div
			class="container mx-auto px-8 mt-8"
			style="animation: slideUp 0.5s ease-out;"
		>
			<div
				class="alert bg-red-50 border-2 border-red-300 backdrop-blur-md shadow-lg p-6"
			>
				<div class="flex items-center gap-4">
					<AlertCircle class="w-8 h-8 text-red-600" />
					<span class="text-red-900 font-medium text-xl"
						>{errorMessage}</span
					>
				</div>
				<button
					class="btn btn-lg btn-ghost text-red-700 hover:bg-red-100"
					onclick={() => (errorMessage = "")}>Close</button
				>
			</div>
		</div>
	{/if}
		...
		{:else}
			...
			<div class="mb-12 text-center">
				...
				<p class="text-2xl md:text-3xl text-gray-600">
					Choose from our premium automated cocktail selection
				</p>
				<div
					class="mt-6 h-1.5 w-32 mx-auto krka-accent-gradient rounded-full"
				></div>
			</div>

			<div
				class="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-10 mx-auto"
				style="animation: fadeIn 0.8s ease-out;"
			>
				<!-- Regular Cocktails -->
				{#each cocktails as cocktail, index}
					<div
						style="animation: slideUp 0.5s ease-out {index *
							0.1}s both;"
					>
						<CocktailCard
							{cocktail}
				...
				<div
					style="animation: slideUp 0.5s ease-out {cocktails.length *
						0.1}s both;"
				>
					<CustomCocktailCard
						onSelect={() => (showCustomModal = true)}
					/>
				</div>
			</div>
		{/if}
...
{#if loading}
	...
	>
		<div
			class="flex flex-col items-center gap-6 bg-white p-12 rounded-3xl shadow-2xl border-2 border-cyan-200"
		>
